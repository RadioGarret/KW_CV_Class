...
//             return uniform random variables in [0,1)
	...
	if(nType == 0){
		...
		for(int i=0; i<nNum; i++){
            ...
            dTemp        = (double)(uiRan) / ( (double)(0xffffffff)+1.0 ); // 0xffffffff = -1 // 0xffffffff=0b1111 1111 1111 1111 1111 1111 1111 1111 = -1
...
{
#ifdef _DEBUG
    assert(_fpRandom != 0); // OnRandom()을 먼저 안해주면 오류뜸.
#endif
    _nIdx    = _nIdx % _nNum; // _nNum으로 나눠주기 때문에 아랫줄의 _fpRandom의 배열 크기(_nNum)를 넘어가는 _nIdx는 생기지 않는다.
