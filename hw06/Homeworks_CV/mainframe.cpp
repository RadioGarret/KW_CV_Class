...
void MainFrame::on_buttonCanny_clicked()
{
    //포커스 된 ImageForm으로부터 영상을 가져옴
    KImageGray   igMain;
    if(_q_pFormFocused != 0 && _q_pFormFocused->ImageGray().Address() &&  _q_pFormFocused->ID() == "OPEN")
    ...
    int lowT = ui->spinLowThreshold->text().toInt();
    int highT = ui->spinHighThreshold->text().toInt();

    mk.CannyEdgeDetection(igMain, igOut, lowT, highT);

    ImageForm* q_pOutput = new ImageForm(igOut, "Canny Edge", this);
    _plpImageForm->Add(q_pOutput);
    q_pOutput->show();
    // UI 활성화 갱신
    UpdateUI();
...
void MainFrame::on_buttonHoughCircle_clicked()
{
    //포커스 된 ImageForm으로부터 영상을 가져옴
    KImageGray   igMain;
    if(_q_pFormFocused != 0 && _q_pFormFocused->ImageGray().Address() &&  _q_pFormFocused->ID() == "OPEN")
    ...
    int R = ui->spinCircleRadius->text().toInt();

    Jeong mk(igMain);

    KImageGray igOut(igMain.Row(), igMain.Col());
    igOut = igMain;
    KImageGray igVoting(igMain.Row(), igMain.Col());

    mk.HoughCircle(igMain, igOut, igVoting, R);


    ImageForm* q_pOutput = new ImageForm(igVoting, "Voting image", this);
    ImageForm* q_pOutput2 = new ImageForm(igOut, "Hough Space", this);
    _plpImageForm->Add(q_pOutput);
    _plpImageForm->Add(q_pOutput2);
    q_pOutput->show();
    q_pOutput2->show();
    // UI 활성화 갱신
    UpdateUI();
...
void MainFrame::on_buttonReadModel_clicked()
{
    printf("put plug.txt file in your project");
}

void MainFrame::on_buttonGeneralizedHough_clicked()
{
    KImageGray igMain;
    ...
    igOut = igMain;
    Jeong mk(igMain);
    mk.GeneralizedHoughTransform(igMain, igOut);

    //출력을 위한 ImageForm 생성
    ImageForm*  q_pForm = new ImageForm(igOut, "Generalized Hough Transform", this);
    _plpImageForm->Add(q_pForm);
    q_pForm->show();
