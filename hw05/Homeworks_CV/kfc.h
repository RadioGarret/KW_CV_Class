...
{
    // OnRandom을 무조건 해줘야하네. 그런데, 의미가 있나? 싶다.
    // _nNum을 선정해주는 과정인데, 꼭 _nNum을 선정해주는게 의미가 있나 싶은거야.
protected:
    double      	_dMean;
    double			_dSigma;

    int				_nNum;	// OnRandom 해줘야 _nNum 할당.
	double      	_dVar,_dVar2;	
    int				_nIdx;  // OnRandom 해줘야 0으로 초기화
    float*      	_fpRandom; // OnRandom(int nNum) 해줘야 nNum만큼 배열이 생성.
	KRandom*    	_opRandom;
	double          _dRandom;
	double          _dConst;
...
public:
    ...
    double&		Generate( );                                        // 숫자 하나 뽑기
    double&		Generate(const double& dMean,const double& dSigma); // 숫자 하나 뽑기
