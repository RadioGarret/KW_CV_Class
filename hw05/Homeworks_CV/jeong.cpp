...
void Jeong::CannyEdgeDetection(const KImageGray igIn, KImageGray &igOut, int lowT, int highT) const
{
    int nHalf = 1;
    ...
    for(int i=nHalf; i<nRow-nHalf;i++)
        for(int j=nHalf; j<nCol-nHalf;j++){
            ...
            // lowT 넘나?
            if(igMag[i][j]>lowT){
                igAng[i][j] = (unsigned short)(oMath.Atan(dGradX, dGradY)+0.5);
                igDir[i][j] = (unsigned char)((((int)(atan2(dGradY, dGradX) * (180.0/_PI)/22.5)+1)>>1) & 0x00000003);
    ...
    while(!v.size()==0){
        ...
        for(int k=0; k<8; k++){
            if(igBuf[i+dx8[k]][j+dy8[k]]==255){
                ...
                v.push_back(make_pair(i+dx8[k],j+dy8[k]));
            }
        }
    }
